...
export default function RiepilogoTab({ cliente, fatture }: { cliente: Cliente; fatture: Fattura[] }) {
  const stats = useMemo(() => {
    ...
    return { fatturato, redditoImponibile, imposta, totaleImposte, incassate, totale: attive.length }
  }, [cliente, fatture])

  const kpis = [
    { label: 'Fatturato netto', value: formatCurrency(stats.fatturato), icon: Euro, color: 'var(--accent2)', note: 'Esclusi rimborsi N1' },
    { label: 'Reddito imponibile', value: formatCurrency(stats.redditoImponibile), icon: TrendingUp, color: 'var(--accent)', note: `Coeff. ${cliente.coefficiente_redditivita}%` },
    ...
    { label: 'Da accantonare', value: formatCurrency(stats.totaleImposte), icon: FileText, color: 'var(--primary-light)', note: 'Imposta + INPS' },
  ]
